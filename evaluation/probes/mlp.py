"""MLP probe training and evaluation."""
...
from .linear import ProbeConfig, ProbeMetrics


class MLPProbeSuite:
    """Suite of MLP probes for multiple label types and layers."""

    def __init__(self, config: ProbeConfig = None, hidden_dim: int = 256):
    ...
    def fit_probes(
        ...
        """Fit MLP probes for all layers and label types.
        ...
        for layer_name, features in activations.items():
            ...
            for label_name, targets in labels.items():
                ...
                probe_metrics = train_mlp_probe(
                    features, targets, self.config, self.hidden_dim
                )
...
def train_mlp_probe(
    ...
    """Train a single MLP probe (1 hidden layer).
    ...
    # Create train/val split (same as linear probe)
