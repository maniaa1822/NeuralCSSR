...
class EpsilonMachine:
    ...
    def generate_sequence(self, length: int, start_state: Optional[str] = None, 
        ...
        for _ in range(total_length):
            # Get all outgoing transitions from current state, now correctly handling non-determinism
            outgoing_transitions = []
            
            for symbol in self.alphabet:
                key = (current, symbol)
                if key in self.transitions:
                    # A single (state, symbol) can lead to multiple next_states
                    for next_state, prob in self.transitions[key]:
                        outgoing_transitions.append((symbol, next_state, prob))

            if not outgoing_transitions:
                # If no outgoing transitions, stop generation
                break
                
            # Sample transition based on probabilities
            # The sum of probabilities for all outgoing transitions from a state should be 1.0
            # We can rely on this assumption if the machine is well-formed.
            
            # Extract symbols, next_states, and probabilities for sampling
            symbols, next_states, probs = zip(*outgoing_transitions)
            ...
            chosen_index = np.random.choice(len(outgoing_transitions), p=normalized_probs)
            
            # Get the chosen symbol and next state
            chosen_symbol = symbols[chosen_index]
            chosen_next_state = next_states[chosen_index]
            
            sequence.append(chosen_symbol)
            current = chosen_next_state
    ...
    def generate_symbol(self) -> str:
        """
        Generate a single symbol from the current state and update the current state.
        ...
        if not outgoing_transitions:
            raise ValueError(f"No outgoing transitions from state {self.current_state}")
        ...
        total_prob = sum(probs)
        if total_prob == 0:
            raise ValueError(f"Zero total probability for transitions from state {self.current_state}")
        ...
        self.current_state = chosen_next_state
    ...
    def compute_causal_state(self, history: List[str]) -> Optional[str]:
        ...
        for symbol in history:
            ...
            if key in self.transitions and self.transitions[key]:
                # For a deterministic causal state machine, there should be only one possible next state
                # for a given current state and symbol. The sum of probabilities for that transition should be 1.0.
                # If there are multiple entries, it implies non-determinism in state transition, which is not
                # expected for causal states. We will proceed assuming the first one is the correct one,
                # but a warning should be issued if the machine is not properly defined.
                if len(self.transitions[key]) > 1:
                    import warnings
                    warnings.warn(f"Non-deterministic transition for key {key}. Taking the first transition.")
    ...
    def set_transition_probabilities(self, probability_map: Dict[str, Dict[str, float]]) -> None:
        ...
        for state_id, symbol_probs in probability_map.items():
            if state_id not in self.states:
                import warnings
                warnings.warn(f"State '{state_id}' in probability_map not found in machine states. Skipping.")
                continue
            ...
            if not np.isclose(total_prob, 1.0):
                import warnings
                warnings.warn(f"Probabilities for state '{state_id}' do not sum to 1.0. Normalizing.")
                for symbol in symbol_probs:
                    symbol_probs[symbol] /= total_prob
    ...
    def compute_stationary_distribution(self) -> Dict[str, float]:
        ...
        try:
            ...
            # Fallback to uniform distribution if eigenvalue decomposition fails
            import warnings
            warnings.warn("Eigenvalue decomposition failed. Falling back to a uniform stationary distribution.")
