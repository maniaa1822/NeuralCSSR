...
UNIFILAR VERSION: Uses the unifilar SevenStateHumanMachine from pysm_generator.py

Inputs:
  - Extracted results JSON (from cssr_enhanced_extractor.py)
  - Ground-truth unifilar seven-state machine (pysm_generator.py)
...
def compute_gt_emissions_unifilar() -> Tuple[List[str], np.ndarray]:
    """Compute ground truth emissions for unifilar SevenStateHumanMachine from pysm_generator.py"""
    # Define the exact emission probabilities from the unifilar implementation
    gt_emissions = {
        'EMPTY': [7/16, 9/16],   # 7/16 → 0, 9/16 → 1
        'AAA': [3/16, 13/16],    # 3/16 → 0, 13/16 → 1
        'AAAB': [7/16, 9/16],    # 7/16 → 0, 9/16 → 1
        'BA': [7/16, 9/16],      # 7/16 → 0, 9/16 → 1
        'BAB': [8/16, 8/16],     # 8/16 → 0, 8/16 → 1 (balanced)
        'BAAB': [7/16, 9/16],    # 7/16 → 0, 9/16 → 1
        'BAA': [3/16, 13/16],    # 3/16 → 0, 13/16 → 1
    }
    
    names = list(gt_emissions.keys())
    emissions = np.array([gt_emissions[name] for name in names])
    return names, emissions
...
def plot_emissions_scatter(output_dir: Path, gt_names: List[str], gt_em: np.ndarray, ex_ids: List[str], ex_em: np.ndarray, mapping: Dict[str, str]) -> None:
    ...
    # Extracted
    plt.scatter(ex_em[:, 0], ex_em[:, 1], c='#1f77b4', alpha=0.7, s=80, label='Extracted states')
    # GT
    plt.scatter(gt_em[:, 0], gt_em[:, 1], c='#d62728', marker='X', s=120, label='GT unifilar states')
    
    # Connect mapped pairs with labels
    ...
    for i, sid in enumerate(ex_ids):
        if sid in inv_map:
            ...
            # Add labels
            plt.annotate(f'{gname}→{sid}', 
                        xy=((ex_em[i, 0] + gt_em[gi, 0])/2, (ex_em[i, 1] + gt_em[gi, 1])/2),
                        fontsize=8, ha='center', alpha=0.7)
    
    # Add state name labels
    for i, name in enumerate(gt_names):
        plt.annotate(name, xy=(gt_em[i, 0], gt_em[i, 1]), xytext=(5, 5), 
                    textcoords='offset points', fontsize=9, color='#d62728')
    for i, sid in enumerate(ex_ids):
        plt.annotate(sid, xy=(ex_em[i, 0], ex_em[i, 1]), xytext=(-5, -5), 
    ...
    plt.ylim(-0.02, 1.02)
    plt.legend()
    plt.title('Emission probabilities: Extracted vs Ground Truth (Unifilar)')
    plt.tight_layout()
...
def plot_alignment_bars(output_dir: Path, errors: Dict[str, float]) -> None:
    ...
    bars = plt.bar(keys, vals, color='#1f77b4', alpha=0.7)
    plt.ylabel('L1 emission error')
    plt.xlabel('GT unifilar state')
...
def plot_state_sizes(output_dir: Path, ex_ids: List[str], counts: np.ndarray) -> None:
    ...
    bars = plt.bar(labels, values, color='#2ca02c', alpha=0.7)
    plt.ylabel('Count')
...
def plot_distance_heatmap(output_dir: Path, gt_names: List[str], gt_em: np.ndarray, ex_ids: List[str], ex_em: np.ndarray) -> None:
    ...
    plt.figure(figsize=(max(8, 0.6 * dist.shape[1]), max(6, 0.8 * dist.shape[0])))
    sns.heatmap(dist, annot=True, fmt='.3f', cmap='viridis_r', 
                xticklabels=ex_ids, yticklabels=gt_names, cbar_kws={'label': 'L1 distance'})
    plt.xlabel('Extracted states')
    plt.ylabel('GT unifilar states')
    plt.title('L1 emission distance matrix (GT ↔ Extracted)')
    plt.tight_layout()
...
def plot_transition_heatmaps(output_dir: Path, ex_ids: List[str], transitions: Dict[str, dict], mapping: Dict[str, str], gt_names: List[str]) -> None:
    ...
    for sym in ['0', '1']:
        ...
        plt.figure(figsize=(max(8, 0.6 * len(order)), max(6, 0.6 * len(order))))
        sns.heatmap(mat, annot=True, fmt='.2f', cmap='Blues', 
                   xticklabels=order, yticklabels=order, cbar_kws={'label': 'Transition probability'})
        plt.xlabel('Next state')
        plt.ylabel('Current state')
        plt.title(f'Extracted transition matrix for token "{sym}" (GT-aligned order)')
        plt.tight_layout()
        plt.savefig(output_dir / f'transition_heatmap_{sym}.png', dpi=200)
        plt.close()


def analyze_unifilar_properties(gt_names: List[str], ex_ids: List[str], ex_info: Dict[str, dict], 
                               ex_trans: Dict[str, dict], mapping: Dict[str, str]) -> Dict:
    """Analyze how well the extracted FSM preserves unifilar properties"""
    analysis = {
        'unifilar_violations': [],
        'determinism_score': 0.0,
        'state_connectivity': {},
        'emission_alignment_quality': {}
    }
    
    # Check for unifilar violations (multiple transitions from same (state, symbol))
    violations = 0
    total_transitions = 0
    for state in ex_ids:
        if state not in ex_trans:
            continue
        for symbol in ['0', '1']:
            if symbol in ex_trans[state]:
                total_transitions += 1
                num_targets = len(ex_trans[state][symbol])
                if num_targets > 1:
                    violations += 1
                    analysis['unifilar_violations'].append({
                        'state': state,
                        'symbol': symbol,
                        'num_targets': num_targets,
                        'targets': list(ex_trans[state][symbol].keys())
                    })
    
    analysis['determinism_score'] = 1.0 - (violations / total_transitions) if total_transitions > 0 else 1.0
    
    # Analyze emission alignment for mapped states
    inv_mapping = {v: k for k, v in mapping.items()}
    gt_emissions_dict = {
        'EMPTY': [7/16, 9/16], 'AAA': [3/16, 13/16], 'AAAB': [7/16, 9/16],
        'BA': [7/16, 9/16], 'BAB': [8/16, 8/16], 'BAAB': [7/16, 9/16], 'BAA': [3/16, 13/16]
    }
    
    for ex_state in ex_ids:
        if ex_state in inv_mapping:
            gt_state = inv_mapping[ex_state]
            ex_probs = ex_info[ex_state]['future_probabilities']
            gt_probs = gt_emissions_dict[gt_state]
            
            l1_error = abs(ex_probs.get('0', 0.0) - gt_probs[0]) + abs(ex_probs.get('1', 0.0) - gt_probs[1])
            analysis['emission_alignment_quality'][ex_state] = {
                'gt_state': gt_state,
                'l1_error': l1_error,
                'ex_p0': ex_probs.get('0', 0.0),
                'gt_p0': gt_probs[0],
                'ex_p1': ex_probs.get('1', 0.0),
                'gt_p1': gt_probs[1]
            }
...
def main():
    parser = argparse.ArgumentParser(description="Visualize CSSR results against unifilar ground truth")
    ...
    gt_names, gt_em = compute_gt_emissions_unifilar()
    ...
    # Unifilar-specific analysis
    unifilar_analysis = analyze_unifilar_properties(gt_names, ex_ids, ex_info, ex_trans, mapping)
    ...
    # Summary with unifilar analysis
    summary = {
        'results_path': str(args.results),
        'ground_truth': 'SevenStateHumanMachine (unifilar)',
        'num_extracted_states': len(ex_ids),
        'num_gt_states': len(gt_names),
        ...
        'unifilar_analysis': unifilar_analysis
    }
    
    with open(out_dir / 'summary.json', 'w') as f:
        json.dump(summary, f, indent=2)

    print(f"Saved unifilar analysis figures to: {out_dir}")
    print(f"Extracted {len(ex_ids)} states, GT has {len(gt_names)} states")
    print(f"Average L1 emission error: {avg_l1:.4f}")
    print(f"Coverage: {coverage:.3f} ({covered}/{total_count} samples)")
    print(f"Determinism score: {unifilar_analysis['determinism_score']:.3f}")
    print(f"Unifilar violations: {len(unifilar_analysis['unifilar_violations'])}")
    
    if unifilar_analysis['unifilar_violations']:
        print("Unifilar violations found:")
        for violation in unifilar_analysis['unifilar_violations'][:3]:  # Show first 3
            print(f"  {violation['state']} --{violation['symbol']}--> {violation['num_targets']} targets")
