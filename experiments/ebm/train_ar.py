...
@torch.no_grad()
def zero_violation_rate(model: AutoRegressiveBinaryLM, tokens: List[int], device: torch.device, context_window: int) -> float:
    total = 0
    violations = 0
    for i in range(1, len(tokens)):
        if tokens[i - 1] == 0:
...
@torch.no_grad()
def evaluate_validation(model: AutoRegressiveBinaryLM, tokens: List[int], device: torch.device, context_window: int, steps: int = 200, batch_size: int = 64) -> float:
    model.eval()
    losses = []
...
@torch.no_grad()
def parity_metrics(model: AutoRegressiveBinaryLM, tokens: List[int], device: torch.device, context_window: int, max_positions: int = 5000):
...
def main():
    ...
    if args.preset == 'golden_mean':
        ...
        default_ckpt = Path('/home/matteo/NeuralCSSR/experiments/ebm/checkpoints/golden_mean_ar.pt')
        default_csv = Path('/home/matteo/NeuralCSSR/experiments/ebm/metrics/golden_mean_ar_metrics.csv')
    ...
    args.ckpt_out = args.ckpt_out or default_ckpt or Path('ar_ckpt.pt')
    args.log_csv = args.log_csv or default_csv or Path('ar_metrics.csv')
